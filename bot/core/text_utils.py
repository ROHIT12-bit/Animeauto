...
class AniLister:
    ...
    async def get_anilist_id(self, mal_id: int = None, name: str = None, year: int = None):
        """Attempt to fetch AniList ID with title and year validation."""
        if mal_id:
        ...
        if res_code == 200 and resp_json.get('data', {}).get('Media'):
            ...
            fetched_title = titles.get('romaji') or titles.get('english') or titles.get('native')
            fetched_year = media.get('startDate', {}).get('year')
            if fetched_title and name:
                if name.lower() not in (fetched_title.lower() or ''):
                    await rep.report(f"Title mismatch: Queried '{name}', got '{fetched_title}'", "warning", log=False)
                    return None
            if year and fetched_year and str(year) != str(fetched_year):
                await rep.report(f"Year mismatch: Queried {year}, got {fetched_year}", "warning", log=False)
                return None
    ...
    async def get_anidata(self):
        ...
        # Fallback to AniList without year
...
class TextEditor:
    ...
    async def get_poster(self):
        ...
        mal_id = self.adata.get("idMal") or (self.adata.get("id") if "mal_id" in self.adata else None)

        # Try to get AniList ID with validation
        anime_id = await self.anilister.get_anilist_id(mal_id=mal_id, name=name, year=year)
        
        if anime_id and str(anime_id).isdigit():
            await rep.report(f"Using AniList poster for ID: {anime_id}", "info", log=False)
            return f"https://img.anili.st/media/{anime_id}"
        
        # Fallback to coverImage with validation
        cover_image = self.adata.get("coverImage", {}).get("large")
        if cover_image:
            if "kitsu.io" in cover_image or "anili.st" in cover_image or "myanimelist.net" in cover_image or "animenewsnetwork.com" in cover_image:
                await rep.report(f"Falling back to coverImage: {cover_image}", "info", log=False)
                return cover_image
            else:
                await rep.report(f"Invalid coverImage source: {cover_image}", "warning", log=False)
        
        # Default image if no valid poster is found
        await rep.report(f"No valid poster found for {self.__name}, using default", "warning", log=False)
    ...
    async def get_caption(self):
        ...
        try:
            ...
            enddate = f"{month_name[month_idx]} {ed['day']}, {ed['year']}" if ed.get('day') and sd.get('year') and month_idx else "N/A"
        except (ValueError, TypeError):
            enddate = "N/A"
        titles = self.adata.get("title", {})
